...
- .NET 10 benchmark snapshots under `docs/benchmarks/results/` (release gate on Cray/Windows at `93ab811`; homelab ARM64 pre-release retained for comparison).
- GitHub-hosted CI for pull requests; self-hosted Windows (Cray) / Linux / macOS only on `push` to `main` and `workflow_dispatch`.
