...
### 2.6 Przykład kompletnej maszyny
...
* **FSM102**: `InvalidHierarchyConfiguration` – parent state bez initial substate ✅
* **FSM103**: `MultipleInitialSubstates` – więcej niż jeden initial dla rodzica ✅

### 3.2 Warnings (ostrzeżenia)

* **FSM104**: `InvalidHistoryConfiguration` – history na non-composite state ✅
* **FSM105**: `ConflictingTransitionTargets` – transition do composite bez explicit child ✅
* **FSM106**: `EqualPriorityTransitions` – przejścia z równym priorytetem (nowa reguła)
...
public string DumpActivePath()
{
    ...
    return sb.ToString(); // e.g., "Work / Work_Active"
}
#endif
```
...
| FSM102 | ⚠️ WARNING | InvalidHierarchyConfiguration - auto-initial gdy brak |
