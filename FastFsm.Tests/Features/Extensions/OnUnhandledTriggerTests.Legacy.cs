...
public class OnUnhandledTriggerTestsLegacy
{
    ...
    [Fact]
    public void Legacy_Unhandled_Fires_OnUnhandledTrigger()
    {
        // Arrange: z Idle nie ma przejścia po Cancel
        var ext = new UnhandledRecordingExtension();
        var machine = new ExtensionsMachineLegacy(ExtState.Idle, new IStateMachineExtension[] { ext });
        machine.Start();
