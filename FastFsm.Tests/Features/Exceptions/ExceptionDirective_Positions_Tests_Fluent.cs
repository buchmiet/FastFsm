...
public class ExceptionDirective_Positions_Tests_Fluent
{
    [Fact]
    public void OnException_InMiddle_Continue_Swallows()
    {
        var m = new MiddlePositionMachine_Fluent(MPState.A);
        m.Start();
        m.Fire(MPTrigger.Go);
    ...
    {
        var m = new EndPositionMachine_Fluent(EPState.A);
        m.Start();
        m.Fire(EPTrigger.Go);
...
public partial class MiddlePositionMachine_Fluent
{
...
public partial class EndPositionMachine_Fluent
{
    private static void Configure() => FSM
        .State(EPState.A)
