...
namespace Tests.SourceGenerators.TypeSystemHelper;

public class TypeSystemHelperTests(ITestOutputHelper output)
{
    private readonly Generator.Infrastructure.TypeSystemHelper _helper = new();


    // See Tests.SourceGenerators/TypeSystemHelper/TypeSystemHelperTests.cs

    [Fact]
    public void TypeSystemHelper_DoesNotCrash_OnFullMultiPayloadMachineTypes()
    {
        // Arrange: Exact types from the problematic machine
        ...
        // Act & Assert: Call all public methods and verify they do not throw
        try
        {
            foreach (var typeName in typesToProcess)
            {
                ...
                // Any of these calls can be a source of stack overflow
                ...
                // Print results to confirm they look sensible
        ...
        {
            // If the test hangs, this block may not run,
            // but if it throws a different exception, we will see it.
            Assert.Fail($"TypeSystemHelper threw an exception: {ex.Message}");
        }

        // If the test reaches this point without hanging,
        // TypeSystemHelper is probably not the problem.
    ...
    public void GetRequiredNamespaces_MixedCases(string input, string[] expected)
    {
        var result = _helper.GetRequiredNamespaces(input)
            .Distinct()
            .OrderBy(s => s)      // compare the ordered list
            .ToArray();
    ...
    public void FormatTypeForUsage_SystemTypeAliases_ReturnsAlias(string input, string expected) => _helper.FormatTypeForUsage(input).ShouldBe(expected);
    ...
    public void FormatTypeForUsage_GenericTypesWithAliases_ReturnsFormattedType(string input, string expected) => _helper.FormatTypeForUsage(input).ShouldBe(expected);

    [Theory]
    [InlineData("OuterClass+InnerClass", "OuterClass.InnerClass")]
    [InlineData("Namespace.Outer+Inner", "Namespace.Outer.Inner")]
    [InlineData("A+B+C+D", "A.B.C.D")]
    public void FormatTypeForUsage_NestedTypes_ReturnsWithDotSeparator(string input, string expected) => _helper.FormatTypeForUsage(input).ShouldBe(expected);
    ...
    public void FormatTypeForUsage_ClrGenericFormat_ReturnsFormattedType(string input, string expected) => _helper.FormatTypeForUsage(input).ShouldBe(expected);
    ...
    public void FormatTypeForUsage_WithGlobalPrefix_ReturnsCorrectFormat(string input, bool useGlobal, string expected) => _helper.FormatTypeForUsage(input, useGlobal).ShouldBe(expected);

    [Theory]
    [InlineData(null, "object")]
    [InlineData("", "object")]
    [InlineData("global::System.String", "string")]
    public void FormatTypeForUsage_EdgeCases_HandlesCorrectly(string? input, string expected) => _helper.FormatTypeForUsage(input).ShouldBe(expected);
    ...
    public void GetNamespace_StandardCases_ReturnsCorrectNamespace(string input, string? expected) => _helper.GetNamespace(input).ShouldBe(expected);

    [Theory]
    [InlineData("MyNamespace.OuterClass+InnerClass", "MyNamespace")]
    [InlineData("A.B.C+D+E", "A.B")]
    public void GetNamespace_NestedTypes_ReturnsContainingNamespace(string input, string expected) => _helper.GetNamespace(input).ShouldBe(expected);

    [Theory]
    [InlineData("System.Collections.Generic.List<T>", "System.Collections.Generic")]
    [InlineData("MyApp.Generic`1[[System.String]]", "MyApp")]
    public void GetNamespace_GenericTypes_ReturnsNamespaceWithoutGenericPart(string input, string expected) => _helper.GetNamespace(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, null)]
    [InlineData("", null)]
    [InlineData("global::System.String", "System")]
    public void GetNamespace_EdgeCases_HandlesCorrectly(string? input, string? expected) => _helper.GetNamespace(input).ShouldBe(expected);
    ...
    public void GetSimpleTypeName_StandardCases_ReturnsTypeName(string input, string expected) => _helper.GetSimpleTypeName(input).ShouldBe(expected);

    [Theory]
    [InlineData("Outer+Inner", "Outer.Inner")]
    [InlineData("A.B+C+D", "B.C.D")]
    public void GetSimpleTypeName_NestedTypes_ReturnsFormattedName(string input, string expected) => _helper.GetSimpleTypeName(input).ShouldBe(expected);

    [Theory]
    [InlineData("List<T>", "List")]
    [InlineData("Dictionary`2", "Dictionary")]
    public void GetSimpleTypeName_GenericTypes_ReturnsNameWithoutGenericPart(string input, string expected) => _helper.GetSimpleTypeName(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, "object")]
    [InlineData("", "object")]
    public void GetSimpleTypeName_EdgeCases_ReturnsObject(string? input, string expected) => _helper.GetSimpleTypeName(input).ShouldBe(expected);
    ...
    public void IsNestedType_PositiveCases_ReturnsTrue(string input, bool expected) => _helper.IsNestedType(input).ShouldBe(expected);

    [Theory]
    [InlineData("System.String", false)]
    [InlineData("MyNamespace.MyClass", false)]
    [InlineData("List<T>", false)]
    public void IsNestedType_NegativeCases_ReturnsFalse(string input, bool expected) => _helper.IsNestedType(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, false)]
    [InlineData("", false)]
    public void IsNestedType_EdgeCases_ReturnsFalse(string? input, bool expected) => _helper.IsNestedType(input).ShouldBe(expected);
    ...
    public void IsGenericType_PositiveCases_ReturnsTrue(string input, bool expected) => _helper.IsGenericType(input).ShouldBe(expected);

    [Theory]
    [InlineData("String", false)]
    [InlineData("System.Collections.ArrayList", false)]
    public void IsGenericType_NegativeCases_ReturnsFalse(string input, bool expected) => _helper.IsGenericType(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, false)]
    [InlineData("", false)]
    public void IsGenericType_EdgeCases_ReturnsFalse(string? input, bool expected) => _helper.IsGenericType(input).ShouldBe(expected);
    ...
    public void EscapeIdentifier_CSharpKeywords_ReturnsEscaped(string input, string expected) => _helper.EscapeIdentifier(input).ShouldBe(expected);
    ...
    public void EscapeIdentifier_NonKeywords_ReturnsUnchanged(string input, string expected) => _helper.EscapeIdentifier(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, null)]
    [InlineData("", "")]
    public void EscapeIdentifier_EdgeCases_ReturnsInput(string? input, string? expected) => _helper.EscapeIdentifier(input).ShouldBe(expected);
    ...
    [InlineData("MyClass", "MyClass")]
    public void FormatForTypeof_SimpleTypes_ReturnsWithGlobalPrefix(string input, string expected) => _helper.FormatForTypeof(input).ShouldBe(expected);

    [Theory]
    [InlineData("List`1", "List<>")]
    [InlineData("Dictionary`2", "Dictionary<,>")]
    [InlineData("Func`3", "Func<,,>")]
    public void FormatForTypeof_GenericTypes_ReturnsSpecialSyntax(string input, string expected) => _helper.FormatForTypeof(input).ShouldBe(expected);

    [Theory]
    [InlineData(null, "object")]
    [InlineData("", "object")]
    public void FormatForTypeof_EdgeCases_ReturnsObject(string? input, string expected) => _helper.FormatForTypeof(input).ShouldBe(expected);
