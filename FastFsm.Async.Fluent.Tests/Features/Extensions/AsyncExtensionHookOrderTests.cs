...
public class AsyncExtensionHookOrderTests
{
    ...
    public async Task Hooks_AreInvoked_InExpectedOrder_OnSuccess()
    {
        ...
        var m = new AsyncHookOrderMachineSuccessFluentFsm(AState.A, new IStateMachineExtension[] { ext });
        await m.StartAsync();
    ...
    {
        var ext = new AsyncRecordingExtension();
        var m = new AsyncHookOrderMachineFailFluentFsm(AState.A, new IStateMachineExtension[] { ext });
        await m.StartAsync();
