...
{
    private static string GenerateLoggingHelper(string ns, string className)
    {
        ...
        void WriteClassBody()
        {
            using (sb.Block($"internal static class {className}Log"))
            {
                void M(string signature, string level, string eventId, string message, string args)
                {
                    sb.WriteSummary("Generated logging helper");
                    using (sb.Block($"public static void {signature}"))
                    {
                        using (sb.Block($"if (logger.IsEnabled(LogLevel.{level}))"))
                        {
                            sb.AppendLine("logger.Log(");
                            using (sb.Indent())
                            {
                                sb.AppendLine($"LogLevel.{level},");
                                sb.AppendLine($"new EventId({eventId}, nameof({signature.Split('(')[0]})),");
                ...
                M("TransitionSucceeded(this ILogger logger, string instanceId, string fromState, string toState, string trigger)",
                  "Information", "1",
                  "State machine {InstanceId} transitioned from {FromState} to {ToState} on trigger {Trigger}",
                  "instanceId, fromState, toState, trigger");
                M("GuardFailed(this ILogger logger, string instanceId, string guardName, string fromState, string toState, string trigger)",
                  "Warning", "2",
                ...
                M("TransitionFailed(this ILogger logger, string instanceId, string fromState, string trigger)",
                  "Warning", "3",
                ...
                M("OnEntryExecuted(this ILogger logger, string instanceId, string methodName, string state)",
                  "Debug", "4",
                  "State machine {InstanceId} executed OnEntry {MethodName} for state {State}",
                  "instanceId, methodName, state");
                M("OnExitExecuted(this ILogger logger, string instanceId, string methodName, string state)",
                  "Debug", "5",
                  "State machine {InstanceId} executed OnExit {MethodName} for state {State}",
                  "instanceId, methodName, state");
                M("ActionExecuted(this ILogger logger, string instanceId, string actionName, string fromState, string toState, string trigger)",
                  "Debug", "6",
                ...
                M("PayloadValidationFailed(this ILogger logger, string instanceId, string trigger, string expectedType, string actualType)",
                  "Warning", "7",
                ...
                M("InternalTransitionOnAncestor(this ILogger logger, string instanceId, string ancestorState, string currentState, string trigger)",
                  "Debug", "10",
                ...
                M("HierarchicalTransition(this ILogger logger, string instanceId, string fromState, string toState, string lcaState, int exitCount, int entryCount)",
                  "Debug", "11",
                ...
                M("CompositeStateEntry(this ILogger logger, string instanceId, string compositeState, string resolvedTarget, string resolutionMethod)",
                  "Debug", "12",
                ...
                M("HistoryRestored(this ILogger logger, string instanceId, string compositeState, string restoredState, string historyType)",
                  "Debug", "13",
                ...
                M("ActivePath(this ILogger logger, string instanceId, string path)",
                  "Trace", "14",
        ...
        {
            using (sb.Block($"namespace {ns}")) { WriteClassBody(); }
        }
        else
        {
            WriteClassBody();
    ...
    private static readonly DiagnosticDescriptor FSM990_LogHelperPreAdd = new(
        "FSM990_PRE",
        "Logging helper pre-AddSource",
        "GenerateLogging={0}; ns='{1}'; class='{2}'; hint='{3}'; first='{4}'",
        "FSM.Generator.AddSource",
        DiagnosticSeverity.Info,
        isEnabledByDefault: true);
    
    private static readonly DiagnosticDescriptor FSM990_LogProps = new(
        "FSM990_PROP",
        "MSBuild logging flags",
        "FsmGenerateLogging={0}; FsmGenerateDI={1}",
        "FSM.Generator.Config",
        DiagnosticSeverity.Info,
        isEnabledByDefault: true);
...
*/";
            ...
            if (model.GenerateLogging)
            {
                ...
                context.ReportDiagnostic(Diagnostic.Create(
                    FSM990_LogHelperPreAdd,
                    candidate.ClassDeclaration.GetLocation(),
                    model.GenerateLogging,
                    model.Namespace ?? string.Empty,
                    model.ClassName,
                    simpleLoggingHint2,
                    firstLine2.Length > 120 ? firstLine2.Substring(0, 120) + "…" : firstLine2));
            ...
            foreach (var (classDeclaration, classSymbol) in classes)
            {
                ...
                try
                {
                    ...
                    if (model.GenerateLogging)
                    {
                        ...
                        string loggingHintName;
                        if (!usedHintNames.Add(simpleLoggingHint))
                        {
                            // Fallback to unique name based on FQN
                            loggingHintName = GetUniqueHintName($"{fqn}Log", usedHintNames);
                        }
                        else
                        {
